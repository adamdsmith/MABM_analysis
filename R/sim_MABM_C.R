...
sim_MABM_C <- function(n_sites = 50,          # number of routes, assumed independent and with non-zero abundance in principle
  ...
  dat <- left_join(dat, site_df, by = "site") %>%
    ...
    mutate(lp = log(base_C) + site_eta + yr_eta +   # Intercepts
  ...
  C_P <- rpois(n = n_sites * n_yrs_surv * n_visits, lambda = exp(dat$lp))
  C_NB <- rpois(n = n_sites * n_yrs_surv * n_visits, lambda = h * exp(dat$lp))
  if (!negbin) C <- C_P else C <- C_NB
  dat$C <- C

  dat <- dat %>%
    select(site, year, visit, C, site_cov, obs_cov) %>%
